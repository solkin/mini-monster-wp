...
namespace mini_monster
{
    ...
    {
        /// <summary>
        /// Список Модулей и их портов.
        /// </summary>
        public static ObservableCollection<Monster> MonstersList = new ObservableCollection<Monster>();

        private static string MONSTER_PROPERTY = "monsters"; 
        
        private static string PROMO_URL = "http://mini-monster.ru/simple/simple.html?fromapp=true";
        ...
        internal void LoadMonsters()
        {
            ...
            if (settings.Contains(MONSTER_PROPERTY))
            {
                App.MonstersList = (ObservableCollection<Monster>)settings[MONSTER_PROPERTY];
            }
            else
            {
                SaveMonsters();
        ...
        internal static void SaveMonsters()
        {
            IsolatedStorageSettings settings = IsolatedStorageSettings.ApplicationSettings;
            if (settings.Contains(MONSTER_PROPERTY))
            {
                settings[MONSTER_PROPERTY] = App.MonstersList;
            }
            else
            {
                settings.Add(MONSTER_PROPERTY, App.MonstersList);
            }
        }

        internal static void AddMonster(string monster_url, string monster_password, string monster_name)
        {
            Monster monster = new Monster(monster_name, monster_url, monster_password);
            MonstersList.Add(monster);
            SaveMonsters();
        }

        internal static void OpenMonsterPromo()
        {
            WebBrowserTask webBrowserTask = new WebBrowserTask();
            webBrowserTask.Uri = new Uri(PROMO_URL, UriKind.Absolute);
            webBrowserTask.Show();
        }
    }
}